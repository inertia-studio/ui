...
function handleSubmit(e: Event) {
    e.preventDefault();
    form.post(`${panelPath.value}/register`, { preserveScroll: true });
}
    ...
    <StudioProvider :panel="panel">
        <AuthLayout :panel="panel">
            <div class="rounded-xl border border-s-border bg-s-surface p-8 shadow-sm">
                <h2 class="mb-6 text-center text-xl font-semibold text-s-text">Create an account</h2>
                ...
                <p class="mt-5 text-center text-sm text-s-text-muted">
                    ...
                    <Link :href="`${panelPath}/login`" class="font-medium text-s-accent hover:underline">Sign in</Link>
                </p>
            </div>
        </AuthLayout>
    </StudioProvider>
